...
with plt.style.context(['science', 'ieee', 'no-latex']):
    pparam = dict(ylabel='AoI', xlabel='Num. Dispositivos')
    fig, ax = plt.subplots()

    # Adicionando curvas com diferentes estilos e cores, ajustando tamanho e espaçamento dos marcadores
    ax.plot(nNodes, AoI_case4, linestyle=estilos[0], color=cores[0], marker=marcadores[0], 
         markersize=4, markevery=4, label='H:3 - CR:1/3')  # Exibe marcador a cada 5 pontos
 
    ax.plot(nNodes, AoI_case3, linestyle=estilos[1], color=cores[1], marker=marcadores[1], 
         markersize=4, markevery=4, label='H:2 - CR:1/2')
 
    ax.plot(nNodes, AoI_case2, linestyle=estilos[2], color=cores[2], marker=marcadores[2], 
         markersize=4, markevery=4, label='H:2 - CR:2/3')
 
    ax.plot(nNodes, AoI_case1, linestyle=estilos[3], color=cores[3], marker=marcadores[3], 
